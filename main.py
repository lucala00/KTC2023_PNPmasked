...
def backward_step_net(z,denoiser,edge_index):
    ...
    data = Data(x=x_in, edge_index=edge_index)
...
def main(inputFolder,outputFolder,categoryNbr):
    ...
    for objectno in range(len(mat_files)):
            ...
            #vm_32 = torch.from_numpy(Uel).to(torch.float32)
            ...
            '''
            fig, ax = plt.subplots()
            cax = ax.imshow(reco_pixgrid, cmap="jet")
            plt.colorbar(cax)
            plt.axis('image')
            '''
            ...
            if MASK==True and categoryNbr>1:
                mat_dictM = spio.loadmat(inputFolderMask +'lev'+str(categoryNbr+1)) #load the reference data
                mask_net = mat_dictM["A"]
                for i in range(256):
                    for j in range(256):
                        if mask_net[i,j]==0:
            ...
            spio.savemat( outputFolder + '/' + str(objectno + 1) + '.mat',mdic)
